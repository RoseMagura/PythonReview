...
def swap_nodes(head, left_index, right_index):
    if (0 <= left_index <= right_index) is False:
        return None
    ...
    while current:
        ...
        if left_index == 0:
            one_current = head
        if counter == left_index - 1:
            one_previous = current
            one_current = current.next
        if counter == right_index - 1:
            ...
            next1 = one_current.next
            next2 = two_current.next
            one_current.next = next2
            two_current.next = next1
        counter += 1
        current = next_node
    return head

def udacity_swap_nodes(head, position_one, position_two):
